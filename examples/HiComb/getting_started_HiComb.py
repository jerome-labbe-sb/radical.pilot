...
#   pprint.pprint(session.get_resource_config(RESOURCE))
    ...
    try:
        ...
        for i in range(0, n):
            ...
            # The following description defines that a unit will stage a file from the local
            # machine to the remote machine (TRANSFER), then execute a copy command, then
            # stage out the output file to another location on the remote machine (MOVE)
        ...
        for i in range(0, n):
            ...
            # create a new CU description, and fill it.
            # Here we don't use dict initialization.
            # The following description defines that a unit will stage a file from the remote
            # machine to another location on remote machine (MOVE), then execute a copy 
            #command, then stage out the output file to local machine machine (TRANSFER)
